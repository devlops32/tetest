# keyboards.py - Клавиатуры (ПОЛНАЯ ВЕРСИЯ)
...
def get_description_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура выбора описания для добавления товара"""
    buttons = [
        [InlineKeyboardButton(text="🔪 príćöp", callback_data="desc_🔪 príćöp")],
        [InlineKeyboardButton(text="🧲 ń@ ḿ@gńite", callback_data="desc_🧲 ń@ ḿ@gńite")],
        [InlineKeyboardButton(text="🎁 t@ÿnik", callback_data="desc_🎁 t@ÿnik")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_quantity")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_description_edit_keyboard(product_id: int) -> InlineKeyboardMarkup:
    """Клавиатура выбора описания для редактирования товара (с ID)"""
    buttons = [
        [InlineKeyboardButton(text="🔪 príćöp", callback_data=f"desc_edit_🔪 príćöp_{product_id}")],
        [InlineKeyboardButton(text="🧲 ń@ ḿ@gńite", callback_data=f"desc_edit_🧲 ń@ ḿ@gńite_{product_id}")],
        [InlineKeyboardButton(text="🎁 t@ÿnik", callback_data=f"desc_edit_🎁 t@ÿnik_{product_id}")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="product_edit_back")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_user_orders_keyboard(orders: List[Dict], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for order in page_orders:
        ...
        keyboard.append([
            InlineKeyboardButton(
                text=f"🛒 {order['product_name']} | {order['price']}₽{desc_text} | {created}",
...
def get_products_list_keyboard(products: List[Dict], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for product in page_products:
        ...
        keyboard.append([
            InlineKeyboardButton(
                text=f"{status} {product['name']} | {product['quantity']} | {product['price']}₽{desc_text}",
...
def get_product_edit_keyboard(product_id: int, in_stock: bool) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="📝 Изменить описание", callback_data=f"product_change_description_{product_id}")],
