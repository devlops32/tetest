...
class Database:
    ...
    def get_user_stats(self, user_id: int) -> Optional[Dict]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                SELECT product_name, city, quantity, price, description, product_code, created_at
                FROM orders 
                WHERE user_id = ?
                ORDER BY created_at DESC
                LIMIT 5
    ...
    def add_product(self, product: Product) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO products (city, name, quantity, price, description, in_stock, product_code)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (product.city, product.name, product.quantity, product.price, product.description, product.in_stock, product.product_code))
            conn.commit()
            return True
    ...
    def add_product_with_campaign(self, product: Product, campaign_id: int = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO products (city, name, quantity, price, description, in_stock, product_code, campaign_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (product.city, product.name, product.quantity, product.price, product.description, product.in_stock, product.product_code, campaign_id))
            conn.commit()
    ...
    def get_products_by_city(self, city: str) -> List[Product]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                SELECT id, city, name, quantity, price, description, in_stock, product_code
                FROM products
    ...
    def get_all_products(self) -> List[Dict]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                SELECT id, city, name, quantity, price, description, in_stock, product_code, created_at
                FROM products
                ORDER BY city, name
            ''')
            results = cursor.fetchall()
    ...
    def add_order(self, order: Order):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO orders (user_id, product_name, city, quantity, price, description, product_code, is_auto)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (order.user_id, order.product_name, order.city, order.quantity, order.price, order.description, order.product_code, order.is_auto))
            conn.commit()
