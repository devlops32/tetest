...
class AdminStates(StatesGroup):
    ...
    waiting_for_photo = State()  # ДЛЯ ОТПРАВКИ ФОТО
...
# ============================================
# ПОДТВЕРЖДЕНИЕ ОПЛАТЫ (С ОТПРАВКОЙ ФОТО)
# ============================================
...
async def payment_confirmed(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем информацию о заказе для отправки фото
    await state.update_data(
        order_id=order.id,
        buyer_id=buyer_id,
...
async def send_photo_start(callback: CallbackQuery, state: FSMContext):
    """Начало отправки фото покупателю"""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔️ Доступ запрещен", show_alert=True)
        return
    ...
    # Получаем информацию о заказе из состояния
    data = await state.get_data()
...
async def send_photo_process(message: Message, state: FSMContext):
    """Обработка отправки фото покупателю"""
    if not is_admin(message.from_user.id):
        await message.answer("⛔️ Доступ запрещен")
        await state.clear()
        return
    ...
    try:
        # Отправляем фото покупателю (без текста)
...
async def close_admin_order(callback: CallbackQuery):
    """Закрытие уведомления о продаже"""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔️ Доступ запрещен", show_alert=True)
        return
    ...
    # Показываем главное меню админа
    keyboard = get_main_menu(True)
    await callback.message.answer(
        "⚡ Привет я современный помощник воспользуйся меню ниже ⬇️",
        reply_markup=keyboard
...
# ... (здесь остальные обработчики: admin_panel, admin_cities, admin_products_menu, 
# admin_products_list, admin_products_add, admin_products_stats, admin_users, 
# auto_sell, и т.д.) ...
